...
from isic_train_test.loader import binary_class
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='/home/hutao/VesselSeg-Pytorch-master/data1/ISIC2018/ISIC2018_Task1-2_Training_Input/',type=str, help='the path of dataset')
    parser.add_argument('--csvfile', default='/home/hutao/BRAU-Netplusplus-master/isic_cvc_train_test/src/test_train_data.csv',type=str, help='two columns [image_id,category(train/test)]')
    parser.add_argument('--model',default='./save_models8/epoch_30_0.7815114409370996.pth', type=str, help='the path of model')
    parser.add_argument('--debug',default=True, type=bool, help='plot mask')
    args = parser.parse_args()
    os.makedirs('debug2',exist_ok=True)
    ...
    model = model_ft.cuda()
    acc_eval = Accuracy(task='binary',num_classes=2)
    pre_eval = Precision(task='binary',num_classes=2)
    dice_eval = Dice()
    recall_eval = Recall(task='binary',num_classes=2)
    f1_eval = F1Score(task='binary',num_classes=2)
    iou_eval = IoU()
    iou_score = []
    acc_score = []
    pre_score = []
    ...
    for image_id in test_files:
        img = cv2.imread(f'/home/hutao/VesselSeg-Pytorch-master/data1/ISIC2018/ISIC2018_Task1-2_Training_Input/images/{image_id}')
        img = cv2.resize(img, (256, 256))
        img_id = list(image_id.split('.'))[0]
        cv2.imwrite(f'debug2/{img_id}.png',img)
    model.eval()
    with torch.no_grad():
        for img, mask, img_id in test_dataset:
            ...
            if args.debug:
                ...
                cv2.imwrite(f'debug2/{img_id}_pred.png', img_numpy)
                mask_numpy = mask_draw.cpu().detach().numpy()[0][0]
                mask_numpy[mask_numpy == 1] = 255
                cv2.imwrite(f'debug2/{img_id}_gt.png', mask_numpy)
