...
class FLB(nn.Module):
    def __init__(
        ...
        self.channel_atten = nn.Sequential(
                nn.Linear(hidden_dim,int(hidden_dim/rate1)),
                nn.ReLU(inplace=True),
                nn.Linear(int(hidden_dim/rate1),hidden_dim)
                )
        self.spatial_atte = nn.Sequential(
                nn.Conv2d(in_channels=hidden_dim,out_channels=int(hidden_dim/rate1),kernel_size=5,padding=2,stride=1),
                nn.BatchNorm2d(int(hidden_dim/rate1)),
                nn.Conv2d(in_channels=int(hidden_dim/rate1),out_channels=hidden_dim,kernel_size=5,padding=2,stride=1))
        self.fc1 = nn.Linear(hidden_dim,hidden_dim)
    def forward(self,x):

        x_chan1 = self.channel_atten(x).permute(0,3,1,2) * x.permute(0,3,1,2)

        x_spat1 = self.spatial_atte(x.permute(0,3,1,2)).permute(0,3,2,1)

        x_spat1 = self.fc1(x_spat1).permute(0,3,1,2)

        x_spat1 = x_spat1 * x.permute(0,3,1,2)

        return x_spat1 * x_chan1

class Channel_Value(nn.Module):

    def __init__(self,hidden_dim,ratio=4):
        super(Channel_Value,self).__init__()

        self.avg_pool = nn.AdaptiveAvgPool2d(1)

        self.fc1 = nn.Sequential(
            nn.Linear(in_features=hidden_dim, out_features=hidden_dim // ratio),
            nn.ReLU(inplace=True),
            nn.Linear(in_features=hidden_dim // ratio,out_features=hidden_dim),
...
class LFEM(nn.Module):
    def __init__(self,in_chans,out_chans,act=False,ratio1=4,wo_dp_conv=False,first=False):
        super(LFEM,self).__init__()

        hidden_dim = int(in_chans * ratio1)
        ...
        layers.extend(dp)
        layers.append(Channel_Value(hidden_dim=hidden_dim,ratio=ratio1))
        layers.extend([
            nn.Conv2d(in_channels=hidden_dim,out_channels=out_chans,kernel_size=1,stride=1,padding=0,bias=False),
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.flb = FLB(hidden_dim=hidden_dim,rate1=4)

    def forward(self, input: torch.Tensor):
        atten = self.self_attention(self.ln_1(input))
        # print(atten.shape)
        x = input.permute(0,3,1,2) + self.drop_path(self.flb(atten))
...
class SwinUMamba(nn.Module):
    def __init__(
        ...
        self.lfem1 = LFEM(in_chans=self.feat_size[1],out_chans=self.feat_size[1])
        ...
        self.lfem2 = LFEM(in_chans=self.feat_size[2],out_chans=self.feat_size[2])
        ...
        self.lfem3 = LFEM(in_chans=self.feat_size[3],out_chans=self.feat_size[3])
        ...
        self.lfem4 = LFEM(in_chans=self.feat_size[4],out_chans=self.feat_size[4])
...
def load_pretrained_ckpt(
    model, 
    ckpt_path="/home/hutao/FasterNet-master/ops/vssmtiny_dp01_ckpt_epoch_292.pth"
):
