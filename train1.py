...
from isic_train_test.loss import *
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            if phase == 'valid' and epoch_loss <= best_loss:
                ...
                counter = 0
                if epoch > 0:
                    torch.save(model.state_dict(), f'./save_models8/epoch_{epoch}_{epoch_acc}.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='/home/hutao/VesselSeg-Pytorch-master/data1/ISIC2018/ISIC2018_Task1-2_Training_Input/', help='the path of images')
    parser.add_argument('--csvfile', type=str, default='./isic_cvc_train_test/src/test_train_data.csv',
    ...
    os.makedirs(f'save_models8/', exist_ok=True)
    ...
    print(df)
    #exit()
    for fold,(train_idx, val_idx1) in enumerate(gkf.split(df, groups=df.image_id.tolist())):
        
        #df.drop(index=df[df['image_id']=='ATTRIBUTION.txt'].index) 
        #exit()
        if train_idx[0] == 1:
            print(df[df['image_id']=='ATTRIBUTION.txt'].index)
            #df.drop(df[df['image_id']=='ATTRIBUTION.txt'].index)
            #exit()
            #df.drop(df[df['image_id']=='ATTRIBUTION.txt'].index)
        if val_idx1 == 'ATTRIBUTION.txt':
            print(val_idx1)
            df.drop(df[df['image_id']==val_idx1].index)
    fold = 0
    ...
    torch.save(model_ft.state_dict(), f'./save_models8/epoch_last.pth')
